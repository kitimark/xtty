...
**Current open changes:** **`add-semantic-capture` (P4a)** — implemented + all 22 tasks done + green (126 `XttyCore` unit + 17 XCUITests), **pending `/opsx:archive`** (its delta specs add `shell-integration` + `terminal-semantics` and modify `terminal-session`/`terminal-multiplexing`/`verification-harness`). P0/P1/P2/**P3a** + **P3b (`add-quick-terminal` + `add-profiles`)** + the verification harness are implemented and archived. **P3 is complete; P4a is the keystone's data half (fork-free).** Outstanding manual-only checks: the quake's real global keypress / multi-monitor / focus-return; and P4a's shell-integration with *uncooperative* host zsh configs (the e2e degrades gracefully there). The architectural seam (all logic talks to the `Terminal` engine via `XttyCore`, never the view) holds — see `research/04-design/01-stack-sketch.md`. Next: archive P4a, then **P4b** (jump-to-prompt / select-output / gutter marks — needs a small upstreamable SwiftTerm fork for absolute row anchors + `SelectionService`; also subsumes the P3b-deferred file:line click-to-open) and the **P8 sidebar** (now unblocked by the block model).
