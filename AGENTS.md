...
- **SwiftTerm submodule (one-time, P4b-2):** SwiftTerm is consumed as a pinned **git submodule** (`external/SwiftTerm` @ `v1.13.0`) with one add-only accessor file dropped in — the no-fork "patch in repo" mechanism (see `research/03-analysis/swiftterm-fork-vs-patch-strategy.md`). After cloning, run **`scripts/bootstrap-swiftterm.sh`** (inits the submodule + drops in `patches/swiftterm/XttyAccessors.swift`). Re-run it after `git submodule update` or editing that patch file. The build fails to resolve `../external/SwiftTerm` until this runs.
...
**Current open changes:** **`add-spatial-blocks` (P4b-2) — implemented & lit up, pending archive** (run `/opsx:archive add-spatial-blocks`). P0/P1/P2/**P3a** + **P3b (`add-quick-terminal` + `add-profiles`)** + the verification harness + **P4a (`add-semantic-capture`)** + **P5 (`add-session-sidebar`)** + **P4b-1 (`add-file-link-open`)** are implemented and archived. P4b-2 is the **first non-fork-free change** — it adds a 2-accessor SwiftTerm addition via a pinned submodule + drop-in (no fork repo; see `research/03-analysis/swiftterm-fork-vs-patch-strategy.md`). **Next: the P6 file/diff view** (needs P4a cwd; reuses P4b-1's path+cwd plumbing), plus the deferred **Tier-2 visual on-screen selection** (forks the view; P7-gated) and the optional **clickable per-block sidebar (P4b-3)**. Sequencing rationale: `research/03-analysis/p5-sidebar-and-p4b-sequencing.md`. Outstanding manual-only checks: the quake's real global keypress / multi-monitor / focus-return; P4a/P5 shell-integration with *uncooperative* host zsh configs (the e2e degrades gracefully there); the sidebar's real cross-tab focus + collapse on a live multi-tab window. The architectural seam (all logic talks to the `Terminal` engine via `XttyCore`, never the view) holds — see `research/04-design/01-stack-sketch.md`.
