...
| `harden-validator-delegation-trigger` | 18/18 — proof captured (**autonomous-spawn**), ready to archive | validator-delegation trigger hardened: iterate/validate boundary + per-task `⟶ xtty-test-validator` marker + `config.yaml` `rules.tasks` seed; §6 by-effect proof held on the post-compact `harden-churn` apply (marked task 3.2 delegated with no user ask; cheap tasks stayed inline) | `openspec/changes/harden-validator-delegation-trigger/` |
