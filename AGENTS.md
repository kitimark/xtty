...
- 📋 **Established specs** (`openspec/specs/`): `app-shell`, `terminal-configuration`, `terminal-keybindings`, `terminal-links`, `terminal-multiplexing`, `quick-terminal`, `terminal-session`, `terminal-semantics`, `shell-integration`, `verification-harness`.
...
**Current open changes:** none archived-pending. P0/P1/P2/**P3a** + **P3b (`add-quick-terminal` + `add-profiles`)** + the verification harness + **P4a (`add-semantic-capture`)** are implemented and archived. **P3 + P4a are complete; P4a is the keystone's data half (fork-free).** **Next: P5 — `add-session-sidebar`** (the favorite feature, H1): a SwiftUI `Tab ▸ Pane` session-progress sidebar driven by P4a's block model — **fully fork-free**; sequenced **before** P4b per `research/03-analysis/p5-sidebar-and-p4b-sequencing.md`. **P4b** (jump-to-prompt / copy-output / file:line click-to-open — needs a small ~2–3-accessor upstreamable SwiftTerm fork; gutter marks dropped) follows P5. Outstanding manual-only checks: the quake's real global keypress / multi-monitor / focus-return; and P4a's shell-integration with *uncooperative* host zsh configs (the e2e degrades gracefully there). The architectural seam (all logic talks to the `Terminal` engine via `XttyCore`, never the view) holds — see `research/04-design/01-stack-sketch.md`.
