...
- 📋 **Established specs** (`openspec/specs/`): `app-shell`, `build-workflow`, `terminal-configuration`, `terminal-keybindings`, `terminal-links`, `terminal-multiplexing`, `quick-terminal`, `terminal-session`, `terminal-semantics`, `shell-integration`, `session-sidebar`, `git-review`, `terminal-spatial-blocks`, `verification-harness`, `research-capture`.
...
**Current open changes:** none. **`add-git-review-tree`** (P6b, Scope A: a flat↔tree toggle over the git-review changed-files list) is implemented **and archived** (213 `XttyCore` unit + 26 XCUITests green). **`add-git-review-polish`** (P6a+: token-level intra-line diff emphasis + a pause-during-own-git poll guard) is implemented **and archived**. **`add-capture-research-tooling`** (the committed `/xtty:capture-research` skill + command + the `.gitignore`/AGENTS capture-and-reconcile workflow → the new `research-capture` spec) is implemented **and archived**. **`add-git-review` (P6a)** is implemented **and archived**; the P6a+ decision is captured in `research/03-analysis/p6-file-diff-decisions.md` → "P6a+ addendum". P0/P1/P2/**P3a** + **P3b (`add-quick-terminal` + `add-profiles`)** + the verification harness + **P4a (`add-semantic-capture`)** + **P5 (`add-session-sidebar`)** + **P4b-1 (`add-file-link-open`)** + **P4b-2 (`add-spatial-blocks`)** + **P6a (`add-git-review`)** + **P6a+ (`add-git-review-polish`)** + **P6b (`add-git-review-tree`)** are implemented and archived. P4b-2 was the **first non-fork-free change** — it adds a 2-accessor SwiftTerm addition via a gitignored upstream clone + `git apply` patch (no fork repo; see `research/03-analysis/swiftterm-fork-vs-patch-strategy.md`). P6a is fork-free again (git via the system binary, no new deps). **Next: P7 (measure) — methodology + scope now explored** (`research/03-analysis/p7-measurement-methodology.md`, 2026-06-29): relative bar vs installed comparators (Warp/iTerm2/Terminal.app), a **fork-free in-process screen-capture latency probe** + `task_info` memory sampling, a base-only `renderer = coregraphics|metal` key for rebuild-free A/B, **distribution/notarization deferred** (0 codesigning identities; orthogonal to the gate); recommended first change **`add-latency-memory-harness` (P7a)** → P7b renderer decision → P7c memory pass. Also still open: the **Tier-2 visual on-screen selection** (forks the view; P7-gated) and the optional **clickable per-block sidebar (P4b-3)**. (P6b's Scope-A changed-files tree shipped in `add-git-review-tree`; the full project file-tree browser, **Scope B**, stays **rejected** as off-mission IDE-creep — see the [P6b addendum](research/03-analysis/p6-file-diff-decisions.md#p6b-addendum--file-tree-scope-decided-scope-a-scope-b-rejected).) P6a+ shipped intra-line emphasis + the pause-during-own-git guard; syntax highlighting / FSEvents / unbalanced-run emphasis remain deferred. Sequencing rationale: `research/03-analysis/p5-sidebar-and-p4b-sequencing.md`. Outstanding manual-only checks: the quake's real global keypress / multi-monitor / focus-return; P4a/P5 shell-integration with *uncooperative* host zsh configs (the e2e degrades gracefully there); the sidebar's real cross-tab focus + collapse on a live multi-tab window. The architectural seam (all logic talks to the `Terminal` engine via `XttyCore`, never the view) holds — see `research/04-design/01-stack-sketch.md`.
