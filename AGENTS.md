...
**Current open changes:** none — P0/P1/P2/**P3a** + **P3b (`add-quick-terminal` + `add-profiles`)** + the verification harness are all implemented and archived (`openspec/specs/` holds `app-shell`, `terminal-configuration`, `terminal-keybindings`, `terminal-links`, `terminal-multiplexing`, `quick-terminal`, `terminal-session`, `verification-harness`; profiles folded into `terminal-configuration` + `terminal-session`). One manual-only check is outstanding for the quake (the real global keypress + multi-monitor summon + focus-return — not assertable in XCUITest). The architectural seam (all logic talks to the `Terminal` engine via `XttyCore`, never the view) and the staged SwiftTerm L3-start decision are in place — see `research/04-design/01-stack-sketch.md`. **P3 is complete.** Next: **P4** (OSC 7 cwd + OSC 133 — the keystone) via `/opsx:propose`; it unblocks file:line error-matching (deferred from P3b) and the P8 sidebar.
