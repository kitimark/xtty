...
**Current open changes:** none — P0/P1/P2/**P3a** + the verification harness are implemented and archived (`openspec/specs/` holds `app-shell`, `terminal-configuration`, `terminal-keybindings`, `terminal-links`, `terminal-multiplexing`, `terminal-session`, `verification-harness`). The architectural seam (all logic talks to the `Terminal` engine via `XttyCore`, never the view) and the staged SwiftTerm L3-start decision are in place — see `research/04-design/01-stack-sketch.md`. Next: scope **P3b** (`add-quick-terminal` + `add-profiles`) with `/opsx:propose` — decisions captured in `research/03-analysis/p3b-shell-ux-decisions.md`.
