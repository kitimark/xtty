...
| `harden-churn-shell-readiness` | implemented — pending archive (12/12) | churn-flake fix: computed-marker readiness gate + `.common`-modes dump timer; churn 5/5 local + full-sweep-validated green (39/1/1 across headless ×2 + graphics); `3.2` autonomously delegated to the validator (the `harden-validator-delegation-trigger` §6 proof) | `research/03-analysis/confirm-close-shell-readiness.md` |
| `harden-validator-delegation-trigger` | 14/18 — implemented, HOLD-OPEN for by-effect proof | validator-delegation trigger hardened: iterate/validate boundary + per-task `⟶ xtty-test-validator` marker + `config.yaml` `rules.tasks` seed; proof captured on the next `harden-churn` apply | `openspec/changes/harden-validator-delegation-trigger/` |
...
| Tooling | local signing identity; CI pipeline; bash-banner silence; wrap-tolerant focus matcher; **menu-clobber fix (`NSApplicationMain`)**; `xtty-test-validator` agent (v4); capture-research tooling + depth bar; **AGENTS.md rules/history split (`slim-agents-context`** — −21.5k startup tokens, probe-verified 36/36 vs fat 34/36**)**; **Metal-renderer retirement (`retire-metal-renderer`** — zero-Metal build, CI guard-free, graphics-VM-validated 39/1/1 of 41; resurrection = P7b methodology**)** | `agents-md-context-budget.md`, `github-actions-ci-cd.md`, `swiftui-mainmenu-clobber-forensics.md`, `claude-code-subagent-execution-forensics.md` |
