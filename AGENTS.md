...
**Snapshot (2026-07-07):** P0–P7 are all implemented and archived — xtty is a daily-drivable native terminal: tabs/splits/windows, quake terminal, profiles, OSC 7+133 semantic capture on auto-injected zsh, session + block sidebars, file-link open, spatial block navigation, git review panel (flat/tree), and the perf harness that settled the renderer question (**keep CoreGraphics**). Test posture: **232 `XttyCore` unit + 42 XCUITests** (post-`harden-findbar-wrap-assertion`); local `make test` green (`41/0/1`) — the churn confirm-close race is **fixed** by `harden-churn-shell-readiness` (computed-marker shell-readiness gate; churn now 5/5 local + green on both VM rigs); VM-rig acceptance envelope **40/1/1 of 42** on a **wide-prompt image that reproduces the hosted runner's prompt width** (`add-vm-prompt-width-parity`), sole benign residual multi-line paste on the rig's bash-3.2; authoritative home `packer/README.md` → Acceptance. CI: `test-core` is the green required gate (Metal-guard-free); `build-and-test` non-blocking — the menu-clobber product bug is fixed and CI-confirmed, and the find-bar marker-wrap **fixed** (`harden-findbar-wrap-assertion`, CI-confirmed find-bar green under the runner's real prompt on run `28841237111`); 1 residual rig fragility remains (bash-3.2 multi-line paste) — a scoped measurement-first pair (`add-zsh-test-image` → `split-shell-dependent-testplan`) will give the shell-dependent tests real coverage on a zsh rig and convert that residual to an honest skip (`research/03-analysis/shell-dependent-test-partitioning.md`).
...
| `add-zsh-test-image` | proposed | zsh login-shell test-image variant (LN-gate neutralized at rig level) to measure the bash-vs-zsh suite divergence; first of a measurement-first pair | `research/03-analysis/shell-dependent-test-partitioning.md` |
| `split-shell-dependent-testplan` | proposed — applies/archives after `add-zsh-test-image` | split XCUITests into Intersection/ShellInteractive plans + honest `XCTSkip`; `bracketedPasteMode` harness field; zsh rig as standing coverage | `research/03-analysis/shell-dependent-test-partitioning.md` |
