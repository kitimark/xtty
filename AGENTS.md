...
| Tooling | local signing identity; CI pipeline; bash-banner silence; wrap-tolerant focus matcher; **menu-clobber fix (`NSApplicationMain`)**; `xtty-test-validator` agent (v4); capture-research tooling + depth bar; **AGENTS.md rules/history split (`slim-agents-context`** — −21.5k startup tokens, probe-verified 36/36 vs fat 34/36**)**; **Metal-renderer retirement (`retire-metal-renderer`** — zero-Metal build, CI guard-free, graphics-VM-validated 39/1/1 of 41; resurrection = P7b methodology**)**; **churn shell-readiness hardening (`harden-churn-shell-readiness`** — computed-marker readiness gate + fail-fast asserted steps + `.common`-mode modal-live dumps; churn F/F/P → 5/5 local + green across both VM rigs**)**; **validator-delegation trigger (`harden-validator-delegation-trigger`** — iterate/validate boundary + per-task `⟶ xtty-test-validator` marker + `config.yaml` `rules.tasks` seed; §6 by-effect proof held: autonomous-spawn on the post-compact `harden-churn` apply**)** | `agents-md-context-budget.md`, `github-actions-ci-cd.md`, `swiftui-mainmenu-clobber-forensics.md`, `claude-code-subagent-execution-forensics.md`, `confirm-close-shell-readiness.md` |
