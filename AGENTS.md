...
**Current open changes:** **`add-git-review-tree`** (P6b, Scope A: a flat↔tree toggle over the git-review changed-files list — view-free `GitFileTree`, a `git-review-layout` config key, a `DisclosureGroup` tree + header toggle, a `layout` dump field) is **implemented — pending archive** (214 `XttyCore` unit + git-review e2e green). **`add-git-review-polish`** (P6a+: token-level intra-line diff emphasis + a pause-during-own-git poll guard) is implemented **and archived**. **`add-capture-research-tooling`** (the committed `/xtty:capture-research` skill + command + the `.gitignore`/AGENTS capture-and-reconcile workflow → the new `research-capture` spec) is implemented **and archived**. **`add-git-review` (P6a)** is implemented **and archived**; the P6a+ decision is captured in `research/03-analysis/p6-file-diff-decisions.md` → "P6a+ addendum". P0/P1/P2/**P3a** + **P3b (`add-quick-terminal` + `add-profiles`)** + the verification harness + **P4a (`add-semantic-capture`)** + **P5 (`add-session-sidebar`)** + **P4b-1 (`add-file-link-open`)** + **P4b-2 (`add-spatial-blocks`)** + **P6a (`add-git-review`)** + **P6a+ (`add-git-review-polish`)** are implemented and archived. P4b-2 was the **first non-fork-free change** — it adds a 2-accessor SwiftTerm addition via a gitignored upstream clone + `git apply` patch (no fork repo; see `research/03-analysis/swiftterm-fork-vs-patch-strategy.md`). P6a is fork-free again (git via the system binary, no new deps). **Next: P6b changed-files tree** (`add-git-review-tree`, **Scope A** — implemented, pending archive; the full project file-tree browser, **Scope B**, stays **rejected** as off-mission IDE-creep — see the [P6b addendum](research/03-analysis/p6-file-diff-decisions.md#p6b-addendum--file-tree-scope-decided-scope-a-scope-b-rejected)), the **Tier-2 visual on-screen selection** (forks the view; P7-gated), the optional **clickable per-block sidebar (P4b-3)**, and **P7 (measure)**. P6a+ shipped intra-line emphasis + the pause-during-own-git guard; syntax highlighting / FSEvents / unbalanced-run emphasis remain deferred. Sequencing rationale: `research/03-analysis/p5-sidebar-and-p4b-sequencing.md`. Outstanding manual-only checks: the quake's real global keypress / multi-monitor / focus-return; P4a/P5 shell-integration with *uncooperative* host zsh configs (the e2e degrades gracefully there); the sidebar's real cross-tab focus + collapse on a live multi-tab window. The architectural seam (all logic talks to the `Terminal` engine via `XttyCore`, never the view) holds — see `research/04-design/01-stack-sketch.md`.
