...
**Current open changes:** **`add-session-sidebar` (P5)** — implemented + all tasks done + green (134 `XttyCore` unit + 18 XCUITests), **pending `/opsx:archive`** (its delta specs add `session-sidebar` and modify `terminal-semantics` + `verification-harness`). P0/P1/P2/**P3a** + **P3b (`add-quick-terminal` + `add-profiles`)** + the verification harness + **P4a (`add-semantic-capture`)** are implemented and archived. **P3 + P4a + P5 are complete (P5 fork-free on P4a's block model).** **Next: archive P5, then P4b** (jump-to-prompt / copy-output / file:line click-to-open — needs a small ~2–3-accessor upstreamable SwiftTerm fork; gutter marks dropped) and the **P6 file/diff view** (needs P4a cwd). Sequencing rationale: `research/03-analysis/p5-sidebar-and-p4b-sequencing.md`. Outstanding manual-only checks: the quake's real global keypress / multi-monitor / focus-return; P4a/P5 shell-integration with *uncooperative* host zsh configs (the e2e degrades gracefully there); the sidebar's real cross-tab focus + collapse on a live multi-tab window. The architectural seam (all logic talks to the `Terminal` engine via `XttyCore`, never the view) holds — see `research/04-design/01-stack-sketch.md`.
