...
| `harden-findbar-wrap-assertion` | implemented — pending archive (pair #2) | test-only: wrap-tolerant matcher at `XttyUITests.swift:198` + `testSoftWrapGuardIsWrapTolerant`; §19b `findbar-marker-wrap` residual→**fixed**. Red→green **proven** across all 3 environments (VM `40/1/1` of 42 red→green, local `41/0/1`, CI find-bar green on run `28841237111`). On-archive: task 5.2 | `research/03-analysis/ci-runner-prompt-width-forensics.md` |
