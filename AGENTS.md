...
**Current open changes:** none — P0/P1/P2 + the verification harness are implemented and archived (`openspec/specs/` holds `app-shell`, `terminal-configuration`, `terminal-session`, `verification-harness`). The architectural seam (all logic talks to the `Terminal` engine via `XttyCore`, never the view) and the staged SwiftTerm L3-start decision are in place — see `research/04-design/01-stack-sketch.md`. Next: scope the **P3 native-shell-UX** milestone with `/opsx:propose`.
