...
- ✅ **P2 `add-daily-driver-baseline`** — implemented (23/23 tasks, **ready to archive**): a view-free `XttyCore` config loader (`~/.config/xtty/config` → font/theme/scrollback/option-as-meta, unit-tested) applied to the live view; bounded scrollback (default 10 000 / ceiling 100 000); Cmd+F find via SwiftTerm's native bar; live Cmd +/−/0 font sizing. Harness (8/8 UI tests) asserts config-applied + bounded-scrollback (new DEBUG state dump), find, and truecolor/emoji/wide (grid dump fixed for CJK + non-BMP emoji). Ligatures are a no-op in SwiftTerm's grid path; SwiftTerm's Metal renderer evaluated and **deferred to the P7 gate** (`research/03-analysis/swiftterm-metal-renderer-spike.md`).
- 📋 **Established specs** (`openspec/specs/`): `app-shell`, `terminal-session`, `verification-harness` (`terminal-configuration` lands on P2 archive).
...
**Current open changes:** `add-daily-driver-baseline` (P2) — implemented and valid, **awaiting archive** (run `/opsx:archive`, which merges the `terminal-configuration` + `terminal-session` deltas into `openspec/specs/`; then complete the post-archive checklist). P0/P1 + the verification harness are implemented and archived. The architectural seam (all logic talks to the `Terminal` engine via `XttyCore`, never the view) and the staged SwiftTerm L3-start decision are in place — see `research/04-design/01-stack-sketch.md`. Next after archiving P2: scope the **P3 native-shell-UX** milestone with `/opsx:propose`.
