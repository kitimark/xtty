...
Acceptance = the measured envelope in the **Current envelope** table below,
row by row per (tier, golden). **A regression is any test going red** —
menu-dispatch, prompt-width (find-bar included), either paste arm, wheel
routing — **or a semantic-capture test reverting to a vacuous pass** (a
`…capture inactive…` attachment without an assertion is itself a defect).
Graphics and headless tiers are expected to match (the `windowCount`
state-dump assertion removed the old graphics-mode focus-steal red).

<!-- MAINTAINER: this table is the repo's SINGLE current test-envelope truth.
     A change that moves any figure OVERWRITES the affected row(s) in place
     and APPENDS exactly one capped entry to "Envelope history" below, in the
     same session. Never append alternative figures around this table, never
     transcribe these figures into any other file (AGENTS.md and the
     xtty-test-validator point HERE), and follow the "Maintenance rules
     (anti-reaccretion)" below. -->

**Current envelope** (one row per tier × golden):

| Tier | Golden / shell | Envelope | Suite | Last measured | Status |
| --- | --- | --- | --- | --- | --- |
| Tier-0 core (`make test-core`) | — (view-free, shell-independent) | `260/0/0` | 260 | 2026-07-26 (`fix-large-diff-memory-bound`) | **CURRENT** |
| Tier-1 local (`make test`, bare metal) | — (local zsh) | `57/0/1` | 58 | 2026-07-27 (`fix-large-diff-memory-bound` task 4.8's post-cross-review clean run) | **CURRENT** |
| Headless VM | `xtty-test:26.5` (bash) | `54/0/1` | 55 | 2026-07-10 (`fix-scroll-reversal-redraw-corruption` full matrix, ×2 samples) | **STALE (3 behind)** |
| Headless VM | `xtty-test-zsh:26.5` (zsh) | `54/0/1` | 55 | 2026-07-10 (same matrix, ×1 sample) | **STALE (3 behind)** |
| Graphics VM | `xtty-test:26.5` (bash) | `54/0/1` | 55 | 2026-07-10 (same matrix) | **STALE (3 behind)** |
| Graphics VM | `xtty-test-zsh:26.5` (zsh) | `54/0/1` | 55 | 2026-07-10 (same matrix) | **STALE (3 behind)** |

Status tokens — **CURRENT**: measured at today's suite size. **STALE (n
behind)**: still that tier's most recent measurement (not a failure), but *n*
tests have been added to the suite since it ran. **SUPERSEDED**: replaced by
a newer measurement — appears only in Envelope history, never in this table.

- **The lone skip** on every XCUITest tier is the opt-in benchmark e2e
  (`XttyPerformanceHarnessUITests`, gated on `XTTY_RUN_BENCH_E2E=1` so
  routine runs never prompt for Screen Recording).
- **The last 5-environment-identical measurement is the 55-test `54/0/1`**
  (2026-07-10, byte-identical failing sets everywhere); the VM tiers are **3
  tests behind across 2 changes** (see the not-yet-covered table below). The
  claim formerly recorded here (and cached in AGENTS.md / HISTORY.md) that a
  *56*-test envelope was identical across all 5 environments was **false** —
  the 56th test's change never ran the VM tiers (corrected 2026-07-28,
  `slim-status-surface` F4).
- **Evidence** — Tier-1:
  `~/Downloads/xtty-vm-poc/artifacts/2026-07-27-tier1-recheck-large-diff-4.8/`
  (the accepted clean isolated run; its first sample,
  `…/2026-07-27-fix-large-diff-memory-bound-task48-tier1-recheck/`, was a
  `56/1/1` whose single red was a non-reproducing, unrelated wrap-geometry
  timing flake — see the archived change's task 4.8). Task 4.4's earlier
  `57/0/1` bundle
  (`…/2026-07-26-fix-large-diff-memory-bound-task44-full-fresh-lifecycles/`)
  predates the cross-review round-1 `GitRunner` edits and is corroborating
  evidence, **not** the accepted figure. VM tiers:
  `…/2026-07-10-fix-scroll-reversal-redraw-corruption/`.
- **Open anomalies** (tracked as Expected-difference-matrix rows below; never
  folded into an envelope figure): the **XCUITest runner-level event-delivery
  stall** (graphics-zsh, 2/2 samples 2026-07-09; did not recur 2026-07-10) —
  see the matrix's stall row — and the single-sample **confirm-close
  readiness race** on `testNewTabOpensAndLastPaneCloseEscalates` (zsh
  headless, 2026-07-09) — see the confirm-close row, item (c).

**Not yet covered on any VM tier** — the 3 tests added after the 2026-07-10
matrix. What a VM run *would* show is a projection, labelled as such, and
never enters the table above:

| Test | Added by | Local status | VM projection (labelled — not an envelope figure) |
| --- | --- | --- | --- |
| `testConfiguredNoWrapModeTogglesAndGeometryMatchesEachMode` | `add-git-diff-wrap-toggle` (2026-07-20) | green, non-vacuous | shell-arm class: real toggle/geometry assertions on zsh legs; capability-absent crisp negative on bash legs |
| `testLargeDiffPreviewBoundsManyLinesAndSingleLine` | `fix-large-diff-memory-bound` (2026-07-26) | green | shell-arm class, same split as its `XttyGitReviewUITests` siblings |
| `testBinaryPreviewDoesNotExecuteConfiguredTextconv` | `fix-large-diff-memory-bound` (2026-07-26) | green | shell-arm class, same split as its `XttyGitReviewUITests` siblings |

#### Maintenance rules (anti-reaccretion)

1. **Overwrite in place.** A new measurement replaces the affected
   Current-envelope row(s); "supersedes"-style prose never accretes around
   the table.
2. **History is append-only.** The replaced figure gets **one** new entry at
   the **top** of Envelope history; existing entries are never edited or
   re-litigated.
3. **Entry cap.** A history entry is one bounded paragraph (≤ ~150 words):
   date · change · suite/envelope movement · what was proven · evidence path
   · mechanism pointer. Deeper narrative lives in `HISTORY.md` and the linked
   research doc, never here.
4. **The matrix carries zero numbers.** The Expected-difference matrix
   records difference *classes* and *causes* only; any tier envelope figure
   (`passed/failed/skipped`) in it is a defect — counts live in the Current
   envelope table and Envelope history, never in the matrix.
5. **Status is a token.** `CURRENT` | `STALE (n behind)` | `SUPERSEDED` —
   never a prose hedge.
6. **Reverse duty names this table, not the changes.** The matrix's
   reverse-duty rule points here; it never enumerates the changes that
   previously updated it.
7. **No current count anywhere else in the repo.** Every other doc
   (AGENTS.md included) points at this table; a transcribed envelope figure
   elsewhere is a drift defect (three cached copies once drifted together
   into the same false 56-test claim).
8. **Projections are labelled and never enter the table.** An
   expected-but-unmeasured figure appears only outside the table, explicitly
   marked as a projection.
9. **Subset runs are never envelope figures.** A focused/single-suite run
   (e.g. the wheel suite alone) is supporting evidence; only a full-suite
   measurement updates a row.

#### Envelope history

Append-only, newest-first, entries capped per rule 3; no entry claims
currency — the Current envelope table above is the only current truth. Full
per-change narratives: `HISTORY.md`.

- **2026-07-26/27 — `fix-large-diff-memory-bound` (core 248 → 260; UI suite
  56 → 58).** Added 12 core accumulator/parser tests and 2 Git-review
  XCUITests (`testLargeDiffPreviewBoundsManyLinesAndSingleLine`,
  `testBinaryPreviewDoesNotExecuteConfiguredTextconv`) with fresh
  app/controller lifecycles, real open-button drives, and exact Git-PID reap
  checks; an independent built-App probe corroborated the memory bound.
  Measured: Tier-0 `260/0/0` (2026-07-26); Tier-1 `57/0/1` of 58, accepted
  on task 4.8's post-cross-review clean isolated run (2026-07-27, HEAD
  `f5ef130`). VM tiers not run — they stayed at the 55-test measurement.
  Evidence: `…/2026-07-27-tier1-recheck-large-diff-4.8/`,
  `…/2026-07-26-fix-large-diff-memory-bound-task44-full-fresh-lifecycles/`,
  `research/artifacts/large-diff-memory/`.

- **2026-07-20 — `add-git-diff-wrap-toggle` (core 237 → 248; UI suite 55 →
  56).** Wrap/no-wrap diff modes + `testConfiguredNoWrapModeTogglesAnd-
  GeometryMatchesEachMode`, driving the real in-panel toggle and asserting
  routing + rendered layout geometry via the DEBUG state dump. Measured
  post-review: Tier-0 `248/0/0`, Tier-1 `55/0/1` of 56 (non-vacuous; 3
  distinct dump attachments). The test is **shell-arm class, not
  shell-independent** (an earlier note here wrongly claimed the opposite,
  caught in the change's own review — its repo/selection setup needs OSC 7
  cwd, zsh-only). VM tiers never ran the 56-test suite. Review ledger:
  `openspec/changes/archive/2026-07-20-add-git-diff-wrap-toggle/`.

- **2026-07-10 — `fix-scroll-reversal-redraw-corruption` (UI suite 54 →
  55).** `testScrollRegionReversalDoesNotCorruptOtherColumns` drives the
  exact `SU`/`SD`-inside-a-scroll-region sequence via `printf`
  (shell-independent; `vim`/`less` never invoke `cmdScrollDown`, `htop` is
  absent from the image); root cause was upstream SwiftTerm's
  `cmdScrollDown` missing its sibling's `marginMode` guard, fixed in
  `patches/swiftterm/xtty-accessors.diff`. Confirmed red pre-fix, green
  post-fix; measured `54/0/1` of 55 on **all 5 environments** (Tier-0
  `237/0/0`; Tier-1 + headless ×2 bash / ×1 zsh + graphics ×2 —
  byte-identical failing sets; the graphics-zsh stall did not recur).
  Superseded the `51/0/1`-of-54 figure without resolving that run's open
  threads. Evidence: `…/2026-07-10-fix-scroll-reversal-redraw-corruption/`;
  mechanism: `research/03-analysis/scroll-reversal-redraw-corruption-forensics.md` §7.

- **2026-07-09 — `smooth-scroll-wheel-momentum` (UI suite 48 → 54).** 6 more
  `XttyMouseWheelUITests`: a real-gesture momentum crisp negative; 3 DEBUG
  synthetic-injection tests, each red→green-proven against its guarded
  regression in isolation; an SGR-bytes-reach-the-child test; the real
  `less --mouse` pager test — all shell-independent. Full-matrix revalidate
  (after fixing a graphics-tier **read-freshness race** — a
  test-synchronization bug, not a product one): `51/0/1` per environment ×5
  identical, all wheel tests green (the 5th/6th tests landed after this
  matrix; wheel-suite-only local runs `11/0/0`/`12/0/0` are subset evidence,
  not envelope figures). The task-4.6 re-run measured the 6th test green on
  all 5 environments but the sweep itself was **OUT-OF-ENVELOPE** on the
  unrelated graphics-zsh runner stall (matrix row below; clone
  `xtty-r54-gfx-zsh-run2-0709` retained). Evidence:
  `…/2026-07-09-smooth-scroll-wheel-momentum-revalidate/`; research:
  `research/03-analysis/scroll-momentum-smoothness-research.md`.

- **2026-07-09 — `fix-scroll-wheel-mouse-reporting` (UI suite 42 → 48).** 6
  wheel-routing XCUITests assert the priority-ordered 3-way branch
  (mouse-report → alt-screen cursor keys → local scrollback, Shift-bypass)
  via the DEBUG `lastWheelRouting` field; gestures ride XCUITest's
  automation channel (the runner has no Post-Event/Accessibility grant — a
  raw CGEvent post is silently dropped), and DECCKM is held deterministically
  against zsh's ZLE. Shell-independent; measured `47/0/1` of 48 on **both
  goldens across all tiers** (Tier-0 `237/0/0`; Tier-1 + headless ×2 +
  graphics ×2 identical, 0 vacuous passes). Evidence:
  `…/2026-07-09-fix-scroll-wheel-fullmatrix/`.

- **2026-07-08 — `split-shell-dependent-testplan` + `harden-paste-wrap-assertion`
  (both goldens `41/0/1` of 42).** The paste test (renamed
  `testMultiLinePasteMatchesShellBracketing`) branches on the observed
  `bracketedPasteMode` and asserts each shell's correct behavior — the bash
  `:87` red became an **asserted green execution arm**, and the
  semantic-capture family's bash arm asserts a **crisp negative** instead of
  vacuous-passing (16 crisp-negative attachments, 0 capture-inactive).
  `harden-paste-wrap-assertion` had first greened the zsh `:82` paste red —
  a prompt-width soft-wrap (not a grid-scrape miss), fixed with the
  wrap-tolerant matcher — zsh rig `40/1/1` → `41/0/1` on both arms. Neither
  paste arm is skipped; both assert. Evidence:
  `…/2026-07-08-split-shell-dependent-testplan-fullmatrix/`,
  `…/2026-07-08-harden-paste-wrap-assertion/`; research:
  `research/03-analysis/shell-dependent-test-partitioning.md`.

- **2026-07-08 — `add-zsh-test-image` (second golden,
  `xtty-test-zsh:26.5`).** The shell-parameterized template produced the zsh
  golden; baseline `40/1/1` of 42 (headless ×2; graphics pre-check the same)
  with the `:82` paste red as the known residual (fixed the same day —
  entry above). The semantic-capture family asserts for real on zsh (0
  capture-inactive), and `fix-osc7-hostname-reverse-dns` holds by effect (0
  `policy 'pending'`, no Local Network modal — see the Local Network section
  above). Evidence: `…/2026-07-08-add-zsh-test-image-headless-matrix/`,
  `…/2026-07-08-graphics-both-goldens/`.

- **2026-07-07 — `add-vm-prompt-width-parity` →
  `harden-findbar-wrap-assertion` (red→green pair; `40/1/1` of 42).** The
  image gained a 59-char single-label `\h` (all three `scutil` keys),
  reproducing the hosted runner's prompt width; applied alone it reproduced
  CI's find-bar wrap flake in-guest (`38/2/1` of 41 interim), then the
  wrap-tolerant focus-restore assertion + a self-validating soft-wrap guard
  flipped it green **on the same image** — `40/1/1` of 42, headless ×2 +
  graphics byte-identical. Measured correction: the long `\h` does **not**
  move the bash paste failure from `:87` to `:84` (contra the §6 option-C
  prediction). Evidence: `…/2026-07-07-findbar-wrap-fix-green-proof/`;
  mechanism: `research/03-analysis/ci-runner-prompt-width-forensics.md`.

- **2026-07-06 — `harden-churn-shell-readiness` + `retire-metal-renderer`.**
  The churn confirm-close race (a fresh split closed before its shell
  settled) was fixed with a computed-marker shell-readiness gate; full
  sweep: Tier-0 `232/0/0`, Tier-1 `40/0/1`, headless ×2 `39/1/1`, graphics
  `39/1/1` — churn green in every tier. `retire-metal-renderer` deleted the
  always-passing Metal e2e (suite 42 → 41; graphics `39/1/1` of 41).
  Evidence: `…/2026-07-06-harden-churn-shell-readiness/`.

- **2026-07-05 — `fix-main-menu-clobber` and the rig's founding purpose.**
  Post-fix: `40/1/1` of 42 (headless ×2 + graphics identical) — the 7
  menu-dispatch tests green. Pre-fix, this rig **reproduced the SwiftUI menu
  clobber** — `34/7/1` ↔ `36/5/1` of 42, the exact CI failing set (the
  per-launch-flaky Cmd+D pair); the first (zsh-shell) image build measured
  `33/8/1` headless / `32/9/1` graphics with **all 8 headless failures
  menu-dispatch class**. That reproduction — impossible on unconstrained
  bare metal — is why the 3-vCPU VM rig exists. Mechanism:
  `research/03-analysis/swiftui-mainmenu-clobber-forensics.md`;
  `github-actions-ci-cd.md` §18.
