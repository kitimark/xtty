...
| **Scroll-reversal redraw corruption (NEW still-open 2026-07-10)** | Real physical trackpad use in a mouse-tracking full-screen app (htop); reproduced deterministically via the DEBUG wheel-injection hook, no hardware needed | Scrolling via xtty's SGR mouse-report path, then **reversing direction** after having scrolled some distance, corrupts the redraw — stale characters from earlier rows bleed into later ones (htop's own CPU%/MEM%/TIME+ keep updating live in the corrupted rows; not a hang). Isolated to the mouse-report path specifically: keyboard arrows (300↓/300↑, exact reversal) clean, keyboard Page Down/Page Up (matched multi-row jumps, exact reversal) clean, the identical wheel recipe against real iTerm2 clean. **Not a `smooth-scroll-wheel-momentum` regression** (character-level redraw corruption, nothing D1–D3's accumulator/momentum logic touches) but **newly reachable** via the wheel→SGR-report feature family (`fix-scroll-wheel-mouse-reporting`) — unreachable before that shipped, so a latent SwiftTerm/xtty rendering defect, not pre-existing-and-unrelated like the confirm-close row above. SwiftTerm-source-level root cause not traced (❓). Full isolation matrix, fates table, and the reusable repro recipe: `research/03-analysis/scroll-reversal-redraw-corruption-forensics.md`. Not yet proposed as a follow-up change; does not block `smooth-scroll-wheel-momentum`'s archival. |
