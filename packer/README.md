...
| **Scroll-reversal redraw corruption (still-open, root-caused 2026-07-10)** | Real physical trackpad use in a mouse-tracking full-screen app (htop); reproduced deterministically via the DEBUG wheel-injection hook or a single scripted `peekaboo` wheel tick each direction, no hardware needed | Scrolling via xtty's SGR mouse-report path toward the **top** of the list corrupts the redraw — stale characters from earlier rows bleed into later ones (htop's own CPU%/MEM%/TIME+ keep updating live in the corrupted rows; not a hang). Isolated to the mouse-report path specifically: keyboard arrows/Page Down/Page Up clean, the identical wheel recipe against real iTerm2 clean. **Not a `smooth-scroll-wheel-momentum` regression** but **newly reachable** via the wheel→SGR-report feature family (`fix-scroll-wheel-mouse-reporting`). **Root cause found and source-confirmed (2026-07-10 addendum):** upstream SwiftTerm `cmdScrollDown` (`Terminal.swift:4646`, CSI Ps T/SD) is missing the `if marginMode {…} else {…}` guard its sibling `cmdScrollUp` has; htop's alt-screen buffer never gets `marginRight` raised off its raw `0` default (`Buffer.init`/`activateAltBuffer` traced — neither sets it), so `columnCount = marginRight-marginLeft+1 = 1` and every scroll-down shifts only column 0, freezing every other column — a byte-exact real capture replayed through a headless SwiftTerm `Terminal` reproduces the identical corruption with zero xtty-app involvement. Full isolation matrix, mechanism, fates table (T1–T12), and reusable repro recipes: `research/03-analysis/scroll-reversal-redraw-corruption-forensics.md` §7. Not yet proposed as a follow-up change (diagnosis settled, fix not drafted); does not block `smooth-scroll-wheel-momentum`'s archival. |
