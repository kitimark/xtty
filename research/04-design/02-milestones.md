...
- 📋 **P4b-3 (`clickable per-block sidebar` — deferred, un-proposed; reframed 2026-06-29):** the fork-free random-access complement to P4b-2's keyboard jump — click a block in the P5 `Tab ▸ Pane` sidebar → focus pane + scroll to it. Explore `p4b-3` + external research (WebSearch sweep + an iTerm2 `/tmp` source clone) **overturned D3's premise** that no keyboard-first terminal ships a clickable command list (iTerm2's Toolbelt does: single-click→scroll-to-mark, shell-integration-gated, a secondary drawer), and confirmed xtty's lazy epoch-bump invalidation is the leaner twin of iTerm2's eager `VT100ScreenMark` recompute (same stale-click→no-op UX). Reframed from "deferred, maybe never" → a cheap, prior-art-validated **secondary** convenience (keyboard stays primary); most machinery already shipped. Open tensions captured: resize→mass-dim of a persistent list (mitigated — durable row fields survive) + the unbounded-`blocks` cap. See the [P4b-3 addendum](../03-analysis/p4b-2-spatial-blocks-decisions.md#addendum-2026-06-29--p4b-3-reframe-the-clickable-block-sidebar-the-d3-no-one-does-this-premise-is-overturned).
