...
## Phase 7 — Polish + MEASURE (decision gate)  ·  M1, M4 *(old P10)*  *(🔬 P7a `add-latency-memory-harness` ✅ archived; P7b `add-trustworthy-latency-probe` ✅ implemented, benched & archived 2026-06-29 → **renderer decided: keep CoreGraphics, skip Phase 8**; P7c researched & scoped 2026-06-29 — `add-lifecycle-census`, not yet proposed)*
...
**P7c researched & scoped (2026-06-29 — `/opsx:explore p7c` + a 14-agent workflow; [`p7c-leak-retain-audit`](../03-analysis/p7c-leak-retain-audit.md); not yet proposed):** the leak/retain pass. **Headline: no leak to hunt** — both explore-flagged retain cycles (`WindowController⇄GitReviewController`, `PaneController⇄LinkRoutingTerminalDelegate`) **refuted** (all closures `[weak self]`, delegates `weak`, `GitReviewTarget` a transient value type), App-layer sweep clean bar 2 cosmetic nits. So P7c reframes to **prove + guard + profile-SwiftTerm**: a DEBUG **live-instance census** (`liveCount` on 6 lifecycle types → a `liveInstanceCounts` state-dump field → a churn XCUITest asserting return-to-baseline = the gated net) + weak-sentinel teardown for in-process `XttyCore` types + a one-time CLI `leaks`/`xctrace`/`vmmap` pass (**not** CI-gated) targeting SwiftTerm's **unbounded glyph/font caches** + incomplete renderer `deinit` (the unnameable-by-census risk). Footprint-after-churn stays **report-only** (too noisy). Recommended change **`add-lifecycle-census`** (fork-free); **drop** the proposed `max-memory-mb` config key. Gates nothing; distribution stays deferred.
