...
## Phase 6 — File / diff view  ·  H2 *(the Zed habit — old P9)*  *(✅ P6a `add-git-review` implemented + archived · 📋 P6a+ `add-git-review-polish` decided — pending proposal)*
...
- 📋 **P6a+ — intra-line diff emphasis (`add-git-review-polish`, decided / pending proposal):** **token-level** word-diff emphasis on the unified diff (tokenize → small LCS/DP over tokens → highlight changed spans via `Text(AttributedString)` per-run background, macOS 12+, no fallback), gated zed/delta-style (≤5 lines / 512 bytes / 1:1 pairing / ratio-gate), pure `XttyCore` + one render swap, zero new deps; bundles a one-line **pause-during-own-git** poll guard. Defers syntax highlighting, FSEvents, unbalanced-run emphasis. See the [P6a+ addendum](../03-analysis/p6-file-diff-decisions.md#p6a-addendum--intra-line-diff-emphasis-decided-add-git-review-polish).
