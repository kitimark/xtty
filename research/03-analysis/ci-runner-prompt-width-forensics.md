...
| T6 | "Bash `\h` in the Tart guest will reflect `scutil --set HostName` (so a long-`\h` VM is trivial)." | ✅ **Resolved 2026-07-07** | Confirmed with a caveat by `add-vm-prompt-width-parity`: P7 warned the effective key varies (bare-metal `\h` tracked `LocalHostName`+`.local` while `HostName` was unset), so the image sets **all three** keys defensively — and in the NAT'd Tart guest that makes bash `\h` read the long 59-char name (grid-verified by effect on 3 clones; `scutil --get HostName` echoes it at build time). Not trivial-by-assumption, but real. The open spike is closed (see the §6 landing addendum). |
...
| **C. Long-`\h` VM fidelity** | Give the Tart image a ~60-char hostname so it faithfully reproduces the runner's prompt width (and *all* prompt-width flakes, not just find-bar). | Makes the VM an honest CI mirror, but **re-baselines the acceptance envelope**: find-bar newly reds (**confirmed** — the intended repro). ~~paste flips its failing line `:87`→`:84`~~ — this predicted paste shift was **refuted by measurement: paste stayed `:87`** (see the landing addendum). Plus the P7 gethostname spike (**now closed — T6**). This *is* the "harness-truthing successor" §19b names. |
...
**Addendum (2026-07-07) — Option C landed: `add-vm-prompt-width-parity`.** A 59-char single-label hostname (`xtty-ci-parity-runner-prompt-width-sjc22-be105-9E54E01C3448`, set on all three `scutil` keys) was baked into the Tart image and the acceptance envelope re-measured on all three VM rigs (headless ×2 + graphics, validator-delegated).

- **Verified by effect** (the D2 precondition, on every clone): bash `\h` renders the long name and a marker typed at the prompt **soft-wraps across ≥2 physical rows** — grid dump `…admin$ AFTERFIND` / `786` (the CI `…$ A` / `FTERFIND####` pattern). This **closes the T6 spike** (§4): in the NAT'd guest, setting all three keys makes `\h` read the long name.
