...
- ✅ **Fix classes that work:** (a) don't run the resolver — bash shell (shipped for the test image as `test-image-bash-shell`) or a non-resolving product API (`gethostname(2)`/`SCDynamicStoreCopyLocalHostName` — proposed, **dropped by owner decision**, test-image scope only; source now pinned to **`gethostname`** to match the shell's `$HOST`, confirmed behavior-preserving — **§8d**; **implemented 2026-07-08 as `fix-osc7-hostname-reverse-dns` (committed `4ebe0cb`), measured GREEN by effect — §9a**); (b) satisfy `getnameinfo` before mDNS — `/etc/hosts` entries for **every local address** (proven: 0 gate events; needs per-boot regeneration, superseded by (a)).
...
- ✅ **(2026-07-08: RESOLVED — see the dated note below)** The product-side trigger *shipped* for real zsh users of xtty on every macOS (one first-launch dialog + a main-thread block on first OSC 7). A non-resolving product fix was proposed (`gethostname` + `SCDynamicStoreCopyLocalHostName` in `localHostNames`), **dropped by owner decision** (2026-07-07, test-image scope only), then **re-proposed and implemented** as `fix-osc7-hostname-reverse-dns`. **(2026-07-07 rationale: behavior-preserving and *more correct* — §8d; T7/T8/T10 close every build-time / rig-level alternative — §8b/§8c/§8e, so the product `gethostname` swap is the only durable path besides running headless.) (2026-07-08: LANDED — the swap is committed (`4ebe0cb`) and measured GREEN by effect on the zsh graphics rig — §9a; the shipping product no longer reaches the reverse-DNS trigger. Full-acceptance archive pending tasks 3.1/4.2/4.3.)**
