...
**And the correction to §11 did not converge either — which is the deeper lesson** ⚠️ The obvious repair (*"revert what §11 added"*) was proposed, then refuted, **three times over**: check (4) turned out to have **three** clauses; **all** are adversarially null (two plain forward commits defeat the confinement clause exactly as they defeat the counter); the confinement clause is **not separable** from the counter (its anchor depends on it); and **deleting the counter deletes the gate's principal *accident* tripwire** — without `dels==0`, a model can silently re-attest and check (2) degrades to *"the value equals itself"*. **The counter's value and its liveness cost are the same property.** So the fix is **genuinely unsettled**, and the forensics doc now **prescribes none** — it records the measurements and the tension, and routes the design to an explore. *That* is the tar pit's real signature: not just that hardening does not converge, but that **the un-hardening does not converge either** while the claim stays mechanical. Full evidence: [`cross-review-gate-defect-forensics.md`](cross-review-gate-defect-forensics.md).
