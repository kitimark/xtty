...
#### Scenario: Block history is bounded
- **WHEN** a session runs more commands than the retained-history maximum
- **THEN** the oldest finished blocks are dropped so the retained list does not exceed the maximum, while the most recent blocks and their ordering are preserved

#### Scenario: Block model runs without the app
- **WHEN** the test suite runs
- **THEN** the block lifecycle (open on C, expose the running block, close on first D, discard prompt-only regions, bound the retained history, record fields) is exercised by a unit test that does not launch the app or create a terminal view
