...
Defines how xtty makes the user's shell emit the semantic sequences that the `terminal-semantics` and `terminal-session` capabilities consume — OSC 133 (command boundaries) and OSC 7 (working directory) — without the user editing any dotfiles. It covers automatic injection for zsh via `ZDOTDIR` redirection (a bundled bootstrap that restores the user's real `ZDOTDIR`, sources their startup files, then installs additive `precmd`/`preexec` hooks that coexist with prompt frameworks), the view-free injection seam in `XttyCore` (the integration directory supplied as a parameter; original `ZDOTDIR` read before the env is reseeded; skipped for `command` one-shots), fail-soft behavior when the resource is missing, and a documented manual fallback. This is the emit side of the P4a keystone; bash/fish auto-injection is out of scope for now.
