...
Clickable links in terminal output: xtty detects both URLs (explicit OSC 8 hyperlinks and implicitly detected `http(s)` URLs) and **file references** (a `file:` URL or a path with an optional `:line[:column]` suffix), indicates them on hover, and on activation routes them — a permitted URL scheme (`http`/`https`/`mailto`) to the system handler, a file reference to the user's editor at the given line, resolved against the focused session's live working directory — never executing clicked text as a shell command. A **scheme guard** blocks any other scheme from being auto-opened. Interception is fork-free: xtty installs a vetted `TerminalViewDelegate` proxy as the view's delegate (it forwards every other delegate method back to SwiftTerm and handles only `requestOpenLink`), because SwiftTerm does not forward `requestOpenLink` to its process delegate and its protocol-extension default cannot be subclass-overridden. The editor command is configurable (`link-opener`) with a `$VISUAL`/`$EDITOR`-inferred default (see `terminal-configuration`).
