# ci-investigation Specification

## Purpose

Defines xtty's **CI-failure investigation** tooling: the committed `xtty-ci-investigator` Claude Code agent (`.claude/agents/xtty-ci-investigator.md`) and its thin `/xtty:investigate-ci` launcher (`.claude/commands/xtty/investigate-ci.md`), which — given a reference to a failed GitHub Actions run — gather the evidence (job status, the triggering commit's changed-file set for a regression pre-check, failing-job logs, and for UI-test reds the `.xcresult` bundle + exported attachments/grid dumps read against the test source) inside an isolated agent context so the invoking session receives only a fixed-skeleton verdict rather than tens of thousands of tokens of log/bundle/attachment noise. It is the mirror of the `test-validation` capability, approached from the opposite direction — it **reads** an already-produced CI run rather than **running** the suite. It covers the runtime deference to a repo-owned classification target (the **CI expected-difference matrix** + required-gate/non-blocking job map in `research/03-analysis/github-actions-ci-cd.md` §19, read fresh each run with a reverse duty on any change that shifts the residual set), the report contract (a definition-version stamp for stale-delivery detection, an overall verdict, the run identity with each failing job marked required-gate/non-blocking and whether the commit touched product/test code, verbatim failures, per-red classification with UNEXPLAINED forbidding an in-envelope verdict, a required-gate red as a stop signal regardless of bucket, evidence paths, and a plain recommendation), the two spawn scenarios (direct user request; apply-loop delegation — gated by the documented watch-vs-investigate boundary and carried by a per-task `⟶ xtty-ci-investigator (<run>)` marker) with the caller's delivery check, and the observe-never-repair guardrails (no code edits, no CI re-run/cancel, verbatim reporting, blocked-prerequisite reporting instead of guessing). Being short and read-only (`gh`/`xcrun`, no VM), it needs no turn-alive/babysitter apparatus — the mechanism the sibling `test-validation` capability requires. Evidence, when kept, lands in a dedicated gitignored `ci-investigations/<run>/` (not `build/`, which stays Xcode-only).
