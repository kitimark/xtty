...
#### Scenario: An open change's status is derived on demand, not cached

- **WHEN** the canonical guide's status surface is read, or a change's tracker-reconcile step runs, for a change that has not yet archived
- **THEN** the guide carries no per-change row transcribing that change's state, remaining work, or blockers
- **AND** the guide instead points to `openspec list` for live state and `openspec show <name>` (or the change's own `tasks.md`) for detail, so the reconcile step never needs to hand-edit a summary that can drift from the checkboxes it would otherwise describe
