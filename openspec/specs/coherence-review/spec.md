...
#### Scenario: An in-scope change is checked for the blocking human-attestation cross-review task

- **WHEN** the agent reviews a change that alters product behavior or the review/verification tooling and whose `tasks.md` tail lacks the blocking human-attestation cross-review task (or whose such task lacks the human-only, model-must-stop wording)
- **THEN** the agent reports a REVIEW (not BLOCKER) finding naming the missing task or wording, while a change that carries it — or a purely mechanical docs/tracker/rename change that needs only the single-agent review — is not flagged
