...
The terminal SHALL let the user split the focused pane horizontally or vertically into multiple panes within a single window, where each pane is an independent terminal session running its own shell. Dragging a split divider SHALL resize the adjacent panes and reflow their shells to the new size without display corruption.
...
The application SHALL support multiple tabs using native macOS window tabbing, where each tab hosts its own pane tree and terminal session(s). Opening a new tab SHALL create a new session, and macOS native tabbing SHALL group tabs (providing the tab bar and standard tab navigation).
