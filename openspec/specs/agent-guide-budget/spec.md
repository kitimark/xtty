# agent-guide-budget Specification

## Purpose
The eagerly-injected agent guide (`AGENTS.md`/`CLAUDE.md` and its import closure) has regrown past 80 KB twice despite an advisory leanness rule existing at three surfaces — each time by *instructed* writing, not a rogue append. This capability bounds it mechanically, below the agent, at the git push boundary: a growth-vs-baseline byte meter over the resolved eager-root set (including symlinks, gitlinks, and their transitive imports), a measured ratchet ceiling that may only be lowered, and a park-don't-lose protocol for refused content. See `research/03-analysis/agents-md-structural-best-practices.md` and `openspec/changes/archive/2026-07-18-bound-the-agents-md-guide/` for the full design and defect-forensics record.
## Requirements
