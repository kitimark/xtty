...
Reproduced live (`research/…/codex-review-integration-forensics.md` §3): a two-question design-soundness brief produced two findings mapping onto exactly those questions, with valid schema output, read-only, xtty untouched. **The claim the test proves:** the brief reaches the model and shapes the review, *and* the structured schema survives — both verified against the real companion, not asserted. **Evidence extended by this change's own cross-review run:** that run's reviewed range was **8 files** (> the ≤2-file inline threshold in `lib/git.mjs`), so Codex was in **summary-only / self-collect** mode — the very regime the proposal cites as the failure of record ("polluted 33–317-file ranges … got neither a brief nor the diff") — and the brief **still engaged** (Pass B's findings cited the briefed claims). So the by-effect evidence now spans both the inlined-diff *and* the summary-only path, not only the small-diff path of the original §3 probe.
...
**No IN-REPO brief file (narrowed by the round-1 dogfood — see below).** The gate tooling excludes exactly `cross-review-ledger.md`; a *new in-repo* brief file would either enter the reviewed-state digest (committed) or trip the digest tool's dirty-tree refusal (untracked) — contradicting the gate-inert claim and task 4.3's "neither gate script changes". So there is **no in-repo brief file**. If a brief would exceed the inline budget, the worker **compacts it and notes the compaction** — it never silently under-briefs, and never spills to a separate *in-repo* file or to the ledger (a dogfood finding: the ledger persists *after* the passes run, so ledger-as-overflow can't reach them).

⚠️ **Corrected by this change's own dogfood (round 1, the delivery layer the original analysis skipped).** The `join`/mangler reasoning above is at the **companion** boundary; the brief also passes through the **shell** the worker builds the `adversarial-review` command in. A brief *about this very tool* contains backticks, `$`, and quotes; built inline as a double-quoted literal it would be **command-substituted / expanded** (executing or altering brief text before `node` runs), and if mis-quoted it **word-splits into multiple positionals** that `positionals.join(' ')` then collapses — destroying the internal newlines the claim relies on. So "delivered with internal newlines intact" holds **only if the brief reaches `node` as one safely-quoted positional** — an unstated precondition the earlier draft missed (the by-effect probe used metacharacter-free prose, so it never exercised this). **The robust idiom — proven by effect on this change's own cross-review run — is to stage the composed brief in an OUT-OF-REPO scratch file (`mktemp`) and read it back via `BRIEF="$(cat …)"`:** out-of-repo ⇒ invisible to git ⇒ never enters the digest or trips the dirty-tree refusal, while `"$(cat)"` delivers the whole file as **one** positional with newlines intact and metacharacters neutralized. So "no brief file" is precisely **no *in-repo* brief file**; an out-of-repo staging file is not merely permitted but is the **prescribed delivery channel** (this also dissolves the ARG_MAX edge the earlier draft waved off as YAGNI).
...
- **R3 — Inline brief delivery is semantic, not byte-identical, and version-sensitive (companion 1.0.6).** → The by-effect probe (a briefed review whose findings reference the brief's questions) is the contract check; re-run it when the resolved companion version changes (design D2). There is **no** file-pointer fallback (dropped — D2).
...
- **⚠️ (Round-2 dogfood, LOGGED not adopted) A shared brief may still correlate the two soundness passes.** The additive *instruction* (D2a) tells each pass to look outside the brief, but both passes still receive identical framing, so there is no structurally-blind slice; and "consensus only when both rest *only* on the same briefed assertion" is hard to adjudicate during a semantic merge. A reviewer proposed **keeping one soundness pass blind** (or blind-first-then-brief), reserving the two-model-consensus flag for an independently-surfaced finding. **Not adopted here** — it reintroduces, by design, the under-briefing this change removes, to protect an *advisory* consensus flag that barely fires and is now human-auditable (the brief is recorded). If the flag proves untrustworthy in **use**, the subtractive answer is to soften/drop the flag, not to redesign briefing. **Decide from usage, not another review round.** **→ Update (round-3 landed-change dogfood): resolved-by-usage.** Both soundness passes independently raised shared-framing (B-1 ∧ C-4), resting only on the shared briefed claim — exactly the case the guard targets — and the worker **correctly did not flag it as consensus**. The guard fired correctly on its first real workload; keep it, and soften only if it ever over-fires. No blind slice reintroduced.
- **(Round-2 dogfood, LOGGED not adopted) Companion-version skew has no persistent trigger.** D2's re-probe lives only in this change's one-time verify task; after archive nothing records the resolved companion version or compares it on later runs, so an upgrade could silently change arg parsing. A reviewer proposed putting the record-and-compare in the **worker protocol** (log the resolved version + probe status in the ledger header — adjacent to the already-recorded effective model — and re-probe on change). **Deferred** to first real use: cheap to add if a version bump ever regresses briefing; premature to build now. **→ Sharpened (round-3 dogfood, B-2, still deferred):** the companion **never echoes the focus text it received**, so there is no *positive* delivery confirmation — a future parser could drop the brief while still returning valid schema, and the ledger would falsely read "briefed". The standing guard remains the **by-effect engagement check** (a briefed reviewer whose findings cite the brief; non-engagement = regression, not a pass); this run passed it on the verified 1.0.6 baseline. Building an enforced version-baseline gate now would be the deferred R2-3 feature — kept logged, not built.
