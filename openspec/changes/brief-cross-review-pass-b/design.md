...
- **⚠️ (Round-2 dogfood, LOGGED not adopted) A shared brief may still correlate the two soundness passes.** The additive *instruction* (D2a) tells each pass to look outside the brief, but both passes still receive identical framing, so there is no structurally-blind slice; and "consensus only when both rest *only* on the same briefed assertion" is hard to adjudicate during a semantic merge. A reviewer proposed **keeping one soundness pass blind** (or blind-first-then-brief), reserving the two-model-consensus flag for an independently-surfaced finding. **Not adopted here** — it reintroduces, by design, the under-briefing this change removes, to protect an *advisory* consensus flag that barely fires and is now human-auditable (the brief is recorded). If the flag proves untrustworthy in **use**, the subtractive answer is to soften/drop the flag, not to redesign briefing. **Decide from usage, not another review round.**
- **(Round-2 dogfood, LOGGED not adopted) Companion-version skew has no persistent trigger.** D2's re-probe lives only in this change's one-time verify task; after archive nothing records the resolved companion version or compares it on later runs, so an upgrade could silently change arg parsing. A reviewer proposed putting the record-and-compare in the **worker protocol** (log the resolved version + probe status in the ledger header — adjacent to the already-recorded effective model — and re-probe on change). **Deferred** to first real use: cheap to add if a version bump ever regresses briefing; premature to build now.
