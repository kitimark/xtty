...
The brief SHALL be delivered through the reviewer's **focus channel** as **inline** text (verified adequate for the design-brief sizes this worker produces). A larger-than-inline brief is **out of scope** for this requirement: rather than introduce a separate brief file (which the gate tooling does not exclude, so it would enter the attested reviewed state), the worker SHALL keep the brief within the inline budget, or carry overflow in an **already-gate-inert** surface (the advisory ledger, which the digest tool already excludes) — never a new file the digest would hash.
