...
The brief SHALL be delivered through the reviewer's **focus channel** as **inline** text, which is adequate for the design-brief sizes this worker produces. A brief SHALL fit the inline budget: if a brief would exceed it, the worker SHALL **compact it and note the compaction** — it SHALL NOT silently under-brief, and SHALL NOT introduce a separate brief file (the gate tooling excludes only the advisory ledger, so any other new file would enter the attested reviewed state).
