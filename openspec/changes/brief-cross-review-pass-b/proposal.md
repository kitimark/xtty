...
- **Single-positional delivery; no IN-REPO brief file.** A separate *in-repo* brief file is unimplementable — the digest tool excludes exactly `cross-review-ledger.md`, so a new worktree file would either enter the reviewed-state digest or trip the dirty-tree refusal. The brief is instead **staged out-of-repo** (`mktemp`, invisible to git) and read into **one** positional via `"$(cat …)"` — the metacharacter-safe, newline-preserving channel (a round-1 dogfood catch: a brief built inline as a double-quoted literal would be command-substituted/word-split, since a brief about this tool carries backticks/`$`/quotes). An over-long brief is **compacted with a noted compaction** — never silently under-briefed, and never spilled to a separate *in-repo* file or the ledger.
...
- **`.claude/commands/xtty/cross-review.md`** — populate the brief on the Pass B invocation (§2 of the command) via the out-of-repo staging idiom (`mktemp` → `"$(cat …)"`, one positional); correct the line-44 focus note; align the delivery wording to *semantic, not byte-identical*. Document the Mode-B rejection (no in-repo brief file — see below).
