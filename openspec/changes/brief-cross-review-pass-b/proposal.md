...
- **Inline delivery is the default**, with a **gate-inert brief-file pointer** fallback only if a brief must lead with `-` or exceed `ARG_MAX` — and if used, the brief file MUST be excluded by `cross-review-digest.sh` and allowlisted by `cross-review-scope.sh`, exactly like the advisory ledger, so it never pollutes the attested reviewed state.
