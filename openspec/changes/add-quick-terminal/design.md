...
`HotKeyParser.parse(_:) -> HotKeySpec?` is pure and `swift test`-able. `HotKeySpec { virtualKeyCode: UInt32; modifiers: ModifierSet; display: String }`.
- **Positional virtual keycodes**, not characters: a fixed name → `kVK_*` table (~60 entries: letters, digits, F-keys, common punctuation incl. `grave`, arrows, space, escape, tab, return). `cmd+grave` ⇒ `kVK_ANSI_Grave` (the physical backtick key, layout-independent). This is why `KeyChord` (character-based) can't be reused directly. The `kVK_*` values are stored as plain `UInt32`, so `XttyCore` needs no Carbon import.
- **Reuse `ModifierSet`** (the existing toolkit-independent modifier model) rather than storing a raw Carbon mask — its own doc already calls out "Carbon masks (the P3b global hotkey) — one model, two adapters." The app-layer `GlobalHotKey` adapter maps `ModifierSet` → Carbon `cmdKey/shiftKey/optionKey/controlKey` at registration time, mirroring how `KeybindAdapter` maps to AppKit menu flags. `fn` is not a standard Carbon modifier ⇒ rejected (it isn't in the modifier vocabulary, so a chord using it fails to parse).
- **Shared grammar:** the modifier-token vocabulary and the structural rule (split on `+`, **≥1 modifier + exactly 1 non-modifier key**, fail-soft) are identical to `KeybindParser`. The split+modifier recognizer is factored into a shared `ChordTokenizing` helper so the two parsers can't drift; they differ only in the final key→target mapping.
