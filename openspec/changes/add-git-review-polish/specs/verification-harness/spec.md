...
#### Scenario: Git-review state is observable

- **WHEN** the focused pane's directory is a git repository with changed files and a file is selected for diff in a `-UITestGridDump` DEBUG build
- **THEN** the state dump reports the repository/remote flags, the changed-file paths and status categories, and the selected file's diff summary (from a cached snapshot, without full diff text) so the test can assert the listing and selection

#### Scenario: Intra-line diff emphasis is observable

- **WHEN** a tracked file with a partial single-line change is selected for diff in a `-UITestGridDump` DEBUG build
- **THEN** the state dump's selected-diff summary reports non-empty intra-line emphasis spans (counts/ranges, without full text) so the test can assert the emphasis was computed
