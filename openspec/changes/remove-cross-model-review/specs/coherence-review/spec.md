...
#### Scenario: A new-symbol placement claim is out of this check's scope, and the gap is not silently covered

- **WHEN** a change's `design.md` or `tasks.md` introduces a genuinely new symbol (a field, type, or function that does not yet exist in the codebase) and places it on a questionable owner
- **THEN** the agent does not flag it under this check (new symbols are exempt from the existing-codebase fact-check), and no other check in this capability substitutes for that coverage
