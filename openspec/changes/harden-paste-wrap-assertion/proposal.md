...
This is **not** the bash paste red. On the bash rig, `testMultiLinePasteIsNotAutoExecuted` reds at `:87` because bash 3.2 has no bracketed paste, so the paste auto-executes — a genuine shell-capability difference owned by `split-shell-dependent-testplan` (`XCTSkipUnless`). The two arms are independent (see design D4).
