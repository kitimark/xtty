...
No turn-alive invariant, no babysitter/resume protocol, no VM serialization — none apply to short read-only runs. Evidence persistence is a **convenience, not a survival mechanism**: the agent MAY drop the downloaded `.xcresult` + a short note under a **dedicated, project-local, gitignored** dir `ci-investigations/<run>/` for human follow-up, but nothing strands if it doesn't. It is a **dedicated** folder on purpose — `build/` is reserved for Xcode/xcodebuild output and must not accumulate unrelated artifacts; CI evidence is disposable and regenerable (re-downloadable from GitHub anytime). *Alternative considered:* `~/Downloads/xtty-ci-poc/<run>/`, mirroring the validator's `~/Downloads/xtty-vm-poc/...` — rejected: temporary POC scaffolding tied to the local test-VM image and slated for removal. *Also rejected:* nesting under `build/ci-investigations/` — that mixes non-Xcode artifacts into the build dir. The launcher's delivery check (definition stamp) is retained; the strand-recovery half is not needed.
...
- **Resolved** — Evidence-dir convention: a **dedicated top-level `ci-investigations/<run>/`**, gitignored on its own line (see D8). Deliberately **not** under `build/` (reserved for Xcode output — keep unrelated artifacts out) and **not** the validator's `~/Downloads/xtty-vm-poc/...` (temporary test-VM-image POC scaffolding slated for deletion).
