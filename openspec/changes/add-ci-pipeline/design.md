...
**D4 — Idempotent Metal guard + verify-on-first-run, instead of trusting preinstall.** *(SUPERSEDED 2026-07-06 by `retire-metal-renderer`: the SwiftTerm patch strips the shader resource, so the build needs no Metal toolchain and both guard steps were removed from `ci.yml`; the missing-build-tool resilience example in this change's `build-workflow` delta is now XcodeGen.)* Add `xcrun -f metal >/dev/null 2>&1 || sudo xcodebuild -downloadComponent MetalToolchain` before building. *Why:* the preinstall evidence is a 2-2 split and the preinstall gate excludes RCs, so a pinned RC Xcode could be exactly the gap. The guard makes the build correct under *both* outcomes at near-zero cost (a no-op when Metal is present). First CI run confirms which case holds (an apply-time verification task). Corollary: prefer the unambiguous release default Xcode over pinning an RC; if pinning, keep the guard.
