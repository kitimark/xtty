...
The worker SHALL locate the external soundness reviewer by a **version-independent, version-ordered** resolution (comparing versions numerically, not lexically). Because a focus hint does **not** restrict what the external reviewer reviews, the worker SHALL scope the review by a **deterministic commit range anchored at an immutable recorded base**: the change SHALL record, **at its creation and before implementation work begins**, the identity of the commit that is its review base — a **single recorded base commit**, requiring no per-commit ownership metadata — and the range SHALL run from that recorded base to HEAD. The review, the reviewed-state digest, and the archive gate's scope classification SHALL all resolve the range from that **same recorded base**; any tooling consuming the base SHALL **fail closed** — refusing with a named error rather than proceeding on a guessed, narrowed, or rewritten range — when the recorded base is **absent, unresolvable, not an ancestor of HEAD, or modified after its introducing commit**. The recorded base SHALL be **immutable**: byte-identical from its introducing commit through HEAD, with no later commit adding, deleting, or rewriting the record — the same since-introduction immutability the attestation line carries (see the archive-gate requirement), extended to the more load-bearing anchor every consumer resolves from — so the reviewed range cannot be silently narrowed by a base rewrite in the window before attestation. Recovery from a modified record SHALL be a deliberate human re-recording (the same human-anchored boundary as recording a base retroactively for a change that predates the tooling), never a silent mechanical pass — and because a re-recording is itself a post-introduction modification of the record, it SHALL NOT restore the record's mechanical immutability standing: the re-anchored change archives only on the archive-gate requirement's **re-anchor boundary** (check 5 there never mechanically re-passes). To keep that recovery performable, the worker and the digest tool SHALL accept an **explicit human-invoked re-anchor acknowledgment** that lifts only the modified-record refusal — never the absent, unresolvable, or non-ancestor arms — and SHALL then name the record's full modification history (every post-introduction commit touching it) loudly in the ledger header and digest output, so the fresh review and attestation the recovery requires can be produced over the re-anchored range while no consumer ever proceeds silently on a rewritten record. The worker SHALL **name that range in the ledger header** — the base, the HEAD, and the reviewed file list — so the reviewed scope is transparent to the human reader rather than asserted by a trusted metadata chain. The worker MAY iterate a **bounded** number of rounds (a single human launch authorizing the whole bounded run), adjudicating and applying fixes between rounds; bounded iteration is **worker behavior only** and carries no archive-eligibility meaning.
...
#### Scenario: The range tooling fails closed on an absent, non-ancestor, or rewritten recorded base

- **WHEN** a change's recorded review base is absent, unresolvable, not an ancestor of HEAD, or has been added to, deleted, or rewritten by any commit after its introducing commit
- **THEN** the range tooling refuses with a named error rather than proceeding on a guessed, narrowed, or rewritten range

#### Scenario: A modified record is reviewable only under an explicit human re-anchor acknowledgment

- **WHEN** a human has deliberately re-recorded a change's review base (recovering from a rewritten or unresolvable record) and needs the fresh review and attestation digest over the re-anchored range
- **THEN** the worker's and digest tool's default invocations still refuse the modified record with a named error, and only the explicit human-invoked re-anchor acknowledgment proceeds — lifting no other fail-closed arm and naming the record's full modification history in the ledger header and digest output — so the recovery is performable without any consumer silently accepting a rewritten record
...
The archive step SHALL, for a change the mechanical classifier deems in scope, evaluate a **fail-closed precondition** whose inputs are the human's attestation line, a digest computed by committed deterministic tooling, and git ground truth — and which **reads no model-authored review artifact** (not a ledger, not a verdict, not a coverage label). To prevent a change from weakening its own gate, the archive step SHALL run the digest and scope tooling **as of the reviewed base state** rather than the working-tree version wherever a prior committed version exists — including when the change *modifies* that tooling, since a modification still has a prior committed version to run from. **Only for a change that itself introduces that tooling does no prior version exist to run from, and the honest trust anchor there is human code review of the committed scripts** — an accepted, stated boundary, not a mechanically-closed one. Archive SHALL be refused unless **all** hold: (1) the human attestation **exists and is well-formed**; (2) the reviewed-state digest, **independently recomputed** over the reviewed content excluding only the review's bookkeeping surfaces (the advisory ledger and the delimited attestation surface), **equals** the attested value; (3) the working tree and index are **clean**; (4) **every commit after the reviewed state touches only bookkeeping surfaces** — in particular the attestation-introducing commit's change to the task file SHALL be confined to the **delimited attestation line and checkbox-state ticks**, altering **no reviewed artifact and no gate implementation**; and (5) the recorded review base is **byte-identical from its introducing commit through HEAD** — no commit after its introduction added, deleted, or rewrote the record. Check (5) extends to the range's anchor the same since-introduction immutability check (4) gives the attestation line, and is the check that reaches the **pre-attestation** window checks (2) and (4) cannot: both recompute over whatever base is recorded when attestation happens, so a base rewritten to a later ancestor *before* attestation would otherwise pass them on the silently-narrowed range under a genuine human tick. A failed check (5) is recoverable only by a deliberate human re-recording of the base and a fresh review and attestation over the re-anchored range. That re-recording is itself a second commit touching the record, so check (5) — **by design** — never mechanically passes again for that change: making it re-passable would require the gate to trust a mechanically-unverifiable this-rewrite-was-authorized marker, which is exactly the narrowing lever check (5) exists to close. A legitimately re-recorded change therefore archives on the **re-anchor boundary** — the single, stated, human-act exception to the all-five rule, the same human-anchored bootstrap class as the tooling-introducing change: checks (1)–(4) are still verified mechanically over the re-anchored range (the digest recomputed under the human's explicit re-anchor acknowledgment — see the worker requirement), check (5) reports its failure naming every post-introduction commit that touched the record, the model SHALL STOP on that report as on any unmet precondition, and only the human — having verified that every named commit is their own deliberate re-recording — MAY proceed past it, on their explicit verification of that visible history, never on a silent mechanical pass and never on a model's judgment. A legitimate re-anchor thus costs a fresh review, a fresh attestation, and a human history-verification — and does not leave the change permanently unarchivable. So neither a reviewed artifact, nor the attestation record, nor the gate tooling, nor the reviewed range's anchor can be mutated — after review or ahead of attestation — under cover of the attestation act. The precondition SHALL be evaluated **immediately before** the archive merge; the mechanical spec-delta merge and tracker reconciliation that follow a passed precondition are the committed archive procedure's **own bookkeeping** — the same committed-procedure trust class as the gate tooling itself, an accepted and stated boundary rather than a re-gated surface — and re-verifying (re-digesting) the final merged candidate before the archive commit is finalized MAY be applied as a hardening option. A design-time **optional strong form** MAY additionally require the attestation act to be cryptographically signed and verify that signature when present; it SHALL NOT be the default.

The canonical project guide SHALL document the **worker protocol** (the passes, the advisory union-with-heuristic-consensus ledger and its header contract, the complete-ledger-every-run surfacing rule, bounded rounds, the one-launch-authorizes-the-bounded-run spend rule, the model-family diversity rule, and the detectable non-gating degradation behavior), the **recorded review-base rule** (recorded at change creation before implementation; immutable — byte-identical from its introducing commit through HEAD; resolved identically by review, digest, and scope classification; fail-closed when absent, unresolvable, not an ancestor of HEAD, or modified after its introduction), the **mechanical path-based scope classifier** (the allowlist, the ambiguous-is-in-scope fail-closed rule, and the rewritten-base-is-in-scope rule), and the **human-attestation gate with its fail-closed archive precondition** (the five checks, the reviewed-base tooling rule and its bootstrap boundary, the exempt-by-act rule, the base-anchor immutability rule, the re-anchor boundary — check 5 never mechanically re-passes; a re-recorded change archives only on the human's verification of the record's history — and the optional-not-default signing form).
...
#### Scenario: A rewritten recorded base cannot flip a change out of scope

- **WHEN** the scope classifier evaluates a change whose recorded review base record was modified after its introducing commit — for example rewritten to a later ancestor so that the narrowed range's changed paths fall entirely inside the allowlist
- **THEN** it classifies the change in scope (fail-closed) regardless of the allowlist patterns, so the rewrite buys no exemption
...
#### Scenario: Archive fails closed on a pre-attestation rewrite of the recorded base

- **WHEN** a commit landing before attestation rewrote the change's recorded review base to a later ancestor, so the digest recomputes cleanly, every post-reviewed-state commit is bookkeeping-only over the narrowed range, and the human's tick is genuine
- **THEN** the archive precondition fails closed on the base-immutability check — the record is not byte-identical from its introducing commit through HEAD — and archive is refused until a human deliberately re-records the base and the change is re-reviewed and re-attested over the re-anchored range, after which it archives only on the re-anchor boundary (never on a mechanical re-pass of check 5)

#### Scenario: A legitimately re-anchored change remains archivable on the re-anchor boundary

- **WHEN** a change's recorded review base became unresolvable through a legitimate history rewrite (or was otherwise honestly invalidated), a human deliberately re-recorded it, and a fresh review and attestation were completed over the re-anchored range
- **THEN** the archive step verifies checks (1)–(4) mechanically over the re-anchored range (the digest recomputed under the human's explicit re-anchor acknowledgment), check (5) still reports the record's post-introduction modification — it never silently passes, and the model stops on it — and archive proceeds only on the human's explicit verification that every commit touching the record is their own deliberate re-recording (the same human-anchored bootstrap boundary as the tooling-introducing change), so the documented recovery terminates in a reachable archive rather than a permanent deadlock
...
- **THEN** it states the worker protocol (passes, advisory ledger and header contract, complete-ledger-every-run surfacing, bounded rounds, one-launch spend, model-family diversity, detectable non-gating degradation), the recorded review-base rule (recorded at change creation; immutable — byte-identical since its introducing commit; resolved identically by review, digest, and scope; fail-closed when absent, not an ancestor, or modified after introduction), the mechanical path-based scope classifier (allowlist + ambiguous-is-in-scope + rewritten-base-is-in-scope), and the human-attestation gate with its fail-closed archive precondition (the five checks, the reviewed-base tooling rule and its bootstrap boundary, exempt-by-act, base-anchor immutability, the re-anchor boundary, and the optional-not-default signing form)
