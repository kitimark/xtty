...
- **THEN** the archive precondition fails closed on the base-immutability check — the record is not byte-identical from its introducing commit through HEAD — and archive is refused until a human deliberately re-records the base and the change is re-reviewed and re-attested over the re-anchored range
