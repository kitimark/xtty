...
### D2 — Build on `ghcr.io/cirruslabs/macos-tahoe-base` at a **pinned version tag**, never `:latest`

`vm_base_name` points at a pinned tag of `macos-tahoe-base`. The base layer is what carries the whole XCUITest infrastructure (§11b): vanilla does kcpassword auto-login + NOPASSWD sudo + screensaver-off; base adds `automationmodetool`, the TCC-database grants, brew, and the runner symlink. Building on base inherits all of it for free — this is precisely why the Tart runs "just worked". Pinning the tag (not `:latest`) keeps the image reproducible; the concrete tag is chosen at apply time (see Open Questions) and recorded in the template.
...
VM settings: keep the base image's nominal disk sizing (the `disk_size` annotation is a max, not the footprint — §11a; the built image materializes ~40 GB); build-time CPU/memory are whatever Packer defaults to. **The race-reproducing 3-vCPU constraint is a run-time property of the test clone** (set when running the clone), not baked into the image.
...
1. **`brew install packer`** (Packer is not currently installed on the dev machine). The `github.com/cirruslabs/tart` plugin is pulled by `packer init` — which can only run once the template (with its `required_plugins` block) exists, so it is not a separate manual step: `make image` runs `packer init` + `packer build` (D8).
2. **Download the official Xcode 26.5 `.xip` with an Apple ID** — `xcodes download 26.5` (or a manual developer.apple.com download) → `~/XcodesCache/Xcode_26.5.xip`. This is the *only* Apple-authenticated step, done once on the host. **Recommendation:** also archive the `.xip` on `/Volumes/savepoint/images/` — insurance against Apple rotating availability, and it makes future rebuilds fully offline.
3. **~45+ GB free disk during the build** (base image pull ~25 GiB + the `.xip` copy + the unxipped Xcode before cleanup), on top of the ~40 GB resulting image — **on the volume that holds the Tart storage (`TART_HOME`)**, where the base pull, the build VM, and the golden image all live. The internal disk measured only 47–86 GiB free (§3/§9g) and the §9 rig already needed `TART_HOME` on the external `savepoint` volume — so that external volume is the likely location; the README records the chosen one so the free-space check is made against the right volume.

## Risks / Trade-offs

- **[Pinned base tag disappears]** cirruslabs may garbage-collect old `macos-tahoe-base` tags → the already-built local image keeps working regardless (the base is only pulled at build time); on a rebuild, bump the pin to the nearest available tag and note it in the README's maintenance section.
...
- **The exact `macos-tahoe-base` tag to pin** — chosen at apply time as the newest versioned tag whose macOS build best matches CI (the rigs ran 26.4/25E246; CI VMs rotate) and recorded in the template + README.
