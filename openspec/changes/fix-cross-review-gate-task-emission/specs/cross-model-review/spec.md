...
The obligation to carry that task SHALL additionally be encoded in the **task-authoring configuration the propose loop reads**, deferring to the guide for the scope boundary. That rule SHALL be an **authoring instruction, not a delegation marker**: it directs what task is *written* and SHALL NOT cause the paid review to fire. Its scope call SHALL be **fail-closed** — the task is emitted when in-scope is uncertain. The **coherence-review** capability SHALL enforce the task's **presence, cardinality, ordering, and human-only wording**, taking blocker force **only** from a mechanical in-scope determination.
...
- **THEN** the task-authoring configuration the loop reads instructs it to emit the blocking human-attestation cross-review task in the tail with its human-only wording — and because that rule is an authoring instruction, emitting the task does not fire the paid external review

#### Scenario: An uncertain propose-time scope call emits the task anyway

- **WHEN** a change's tasks are authored while its implementation has not yet landed, so the mechanical classifier cannot yet report it in scope and the authoring scope call is a semantic prediction
