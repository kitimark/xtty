...
- **Migration.** `add-git-diff-wrap-toggle` gets the missing human-attestation task. **`add-ci-pipeline` is deliberately excluded** — it has no `⟶ xtty-openspec-critic` task to anchor against, its digest range is ~315 files (98.4% foreign; HEAD-dependent — it grows with every commit), and its remaining owner-step commits would restage the HEAD-dependent digest while check (4) forbids re-recording, making the task **unsatisfiable**. It is already `exit 10`, so the archive gate applies to it either way — excluding it does not make it *un-archivable* (step-0's check (1) greps the attestation **line**, not the gate **task**). But it does create a **standing, unclearable critic BLOCKER** on `add-ci-pipeline` until the base-resolution change disposes of it — **an accepted residual, recorded in design D7, not a costless exclusion.**
...
- `coherence-review`: the **cross-review-gate-task check** changes from a heuristic REVIEW-only check to one whose severity follows the authority of the scope call — a **mechanical BLOCKER** when the committed classifier resolves the change **in scope** (covering an absent, duplicated, misordered, or mis-worded task); a **semantic REVIEW that is never suppressed** when it resolves the change **out of scope** (`exit 0` — *not yet knowable to be in scope*, the normal post-propose state) or cannot resolve an uncommitted change; and a **blocked-prerequisite** when it refuses on a committed malformed range.
