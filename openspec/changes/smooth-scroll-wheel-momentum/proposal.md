...
- `verification-harness`: the **Mouse-wheel routing is observable** requirement (and its end-to-end coverage) extends the wheel-routing dump to record the momentum/precise nature of the routed event; a new **Mouse-wheel momentum routing coverage via synthetic injection** requirement adds a DEBUG injection hook that drives constructed momentum events through the real router to assert momentum-not-dropped / lossless-carry / `.began`-reset — leaving only the real-OS coast delivery/rate/back-pressure as a manual physical-trackpad verify.
...
- **Tests**: `AppUITests/XttyMouseWheelUITests.swift` gains a crisp-negative (real gesture → non-momentum) **and** three injection-based momentum tests (not-dropped, lossless-carry, `.began`-reset); the existing routing branches are untouched (synthetic events never hit the removed guard). The suite grows (48 → the new total), so **`packer/README.md`'s acceptance envelope updates in the same session** (before the validator runs).
