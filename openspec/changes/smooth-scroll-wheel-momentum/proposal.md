...
- **Tests**: `AppUITests/XttyMouseWheelUITests.swift` gains a crisp-negative (real gesture → non-momentum) **and** four injection-based momentum tests (not-dropped, lossless-carry, `.began`-reset, and a byte-level SGR mouse-report regression — the last added after the human manual verify succeeded, closing the gap between DEBUG-dump bookkeeping and the real byte stream a child process receives); the existing routing branches are untouched (synthetic events never hit the removed guard). The suite grows (48 → the new total), so **`packer/README.md`'s acceptance envelope updates in the same session** (before the validator runs).
