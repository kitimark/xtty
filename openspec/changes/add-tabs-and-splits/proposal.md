...
- **Code:** `App/TerminalWindowController.swift` (decomposed into a window/tab controller + new `PaneController`), `App/MainMenu.swift` (split/close/focus/new-tab items; font actions re-routed to the responder chain), `App/XttyApp.swift` (multi-window lifecycle, `newWindowForTab`), `App/TerminalConfigurator.swift` (per-pane apply). New `XttyCore` types (pane/split-tree model + session registry). Harness support + tests in `AppUITests/`.
