...
- [ ] 3.3 Post-merge: watch the next `build-and-test` run (`gh run watch` — inline) and confirm the find-bar red flips green under the runner's real long-prompt wrap; `test-core` stays green. If it is unexpectedly still red, investigate the run ⟶ xtty-ci-investigator (<run>).
...
- [ ] 4.2 `packer/README.md` Acceptance: refresh the "why the CI `findbar-marker-wrap` residual is NOT in this envelope" note to reflect that the assertion is now wrap-tolerant (fixed); the VM envelope count is unchanged (find-bar already passed on the short-`\h` VM).
