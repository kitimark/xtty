...
- [ ] 4.1 Commit the package and tooling before linking — the symlink must resolve a committed directory, and a clean pushed tree is the precondition for any agent run
- [ ] 4.2 Run `make design-link` against the running app; confirm from its output the link resolves to the repo path, the catalogue reports the package published, and token coverage is complete
