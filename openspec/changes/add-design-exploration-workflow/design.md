...
The app-side project is named **`xtty`**, not after its folder. The folder does not follow: the project folder stays `design/mockups/` (the package folder is `xtty-design-system/` — identity-bearing, D10 — and the generic `mockups/` working-folder name is an owner readability call) and the name/folder mismatch is accepted. It is sound because the name is pure display in the tool — it labels the project list, the export archive filename, and the seeded conversation title; it never reaches the agent's prompt, never keys an artifact path (folder-backed projects write into `baseDir`), and the script resolves projects exclusively by `realpath(metadata.baseDir)`.
