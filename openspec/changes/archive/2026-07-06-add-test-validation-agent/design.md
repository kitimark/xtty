...
- [Foreground tool timeouts kill long tiers] → Claude Code's Bash tool defaults to 120 s (600 s hard max) — below every tier ≥1 (local ~5 min; VM ~7 min/run + boot). The agent prompt mandates background execution (`run_in_background`/nohup → log file) with sparse 30–60 s polling, which also bounds the main in-agent context-growth source over a 25–35 min sweep. **Amended at apply (v4, 2026-07-06):** live smokes + a 7-agent forensic workflow measured that `run_in_background` is the *strand mechanism* for a subagent — its "re-invokes you when it exits" promise is false for subagents (task closes on turn end; three sweeps stranded), so the shipped prompt **bans `run_in_background`** and mandates the control-run-proven unfused shape instead: instant foreground `nohup` launch (+ pidfile + `ledger.log` line) → separate foreground bounded wait loops (`while kill -0 … sleep 30`, `timeout` 600 s, reissued on timeout), with a wedge escape (blocker report after 15 min zero-progress) and a caller-side babysitter/resume protocol in the launcher. A per-report `Definition: v4` stamp guards against Claude Code's session-cached agent definitions (edits need a fresh session — probe-proven). A PreToolUse deny-hook on `run_in_background` was evaluated and deferred (unverified whether hooks fire for subagent calls; `.claude/settings.json` tracking policy unresolved).
