...
**D-restart — `make restart` = terminate the running instance, then `open` the installed bundle.** Backs Requirement 3 (the reinstall/relaunch helper), so the build → install → run loop is one step. Implemented as `pkill -x xtty` (best-effort; a no-op when nothing is running) followed by `open "$(INSTALL_DIR)/xtty.app"`. It also serves the D2 mitigation — quit the running app before reinstalling over it, avoiding a half-copied bundle. *Alternative considered:* an AppleScript `quit` (graceful) — rejected as heavier, and xtty exposes no scripting dictionary; `pkill` is reliable and dependency-free.
