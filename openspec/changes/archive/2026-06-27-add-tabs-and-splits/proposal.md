...
- **Clickable URL links.** Surface SwiftTerm's existing OSC 8 + implicit URL detection (hover highlight + open on click is already wired to `NSWorkspace.open`). The non-`http(s)` confirmation **guard is deferred** (SwiftTerm's link dispatch is a non-overridable protocol-extension default — design D7). Bare `file:line` matching is **not** free in SwiftTerm and is also deferred (see Impact).
...
- `terminal-links`: clickable links in terminal output — surfacing SwiftTerm's URL/OSC-8 detection + open via the system handler. (The non-`http(s)` confirmation guard is **deferred** — SwiftTerm dispatches link opening through a non-overridable protocol-extension default; see design D7. file:line error-matching and open-in-editor remain out of scope.)
