...
### D4 — Rendering: recursive `DisclosureGroup`, default-expanded via a *collapsed* set
**As shipped** (this supersedes the original `OutlineGroup` sketch — see below): a small recursive `GitFileTreeView` renders `[GitTreeNode]` inside the existing `List` (tree mode replaces the `ForEach`-over-categories branch). Each directory is a `DisclosureGroup` whose `isExpanded` binds to a window-level `@State collapsedDirs: Set<String>` keyed by the node's stable cumulative path (D1) — stored as the *collapsed* set so the **default (empty) is all-expanded**, new directories appear expanded, and collapse intent persists across snapshot refreshes (stable ids). `collapsedDirs` is reset on `repoRoot` change (collapse intent is per-repository). Leaves are the shared `GitFileButton` (the same selection `Button { onSelect } .listRowBackground(selected) .contextMenu { onOpen }` the flat rows use) → **selection + ⌘-click-open work identically** (spec scenario), zero new routing.

> **Why not `OutlineGroup(children:)`** (the original plan): `OutlineGroup` defaults to *collapsed* with no clean "expand-all", and the design's Risks section pre-authorized the explicit-`@State` fallback for exactly this. The recursive `DisclosureGroup` + collapsed-set gives default-expanded for free and robust cross-refresh persistence, so it shipped instead. `GitTreeNode` therefore carries no `children` convenience accessor (it would only have served `OutlineGroup`); the tree view pattern-matches the enum directly.
