...
- **Tighten the `terminal-spatial-blocks` spec wording** (a residual the audit missed, caught during verification): its mechanism-neutral "means of injecting the addition" example said "add-only **drop-in** file/patch" — the requirement gets a MODIFIED delta removing "drop-in" while staying neutral, and the spec's Purpose is hand-fixed at archive (deltas don't carry Purpose).
...
- `terminal-spatial-blocks`: the "Engine-only scroll-invariant coordinate provider" requirement's mechanism-neutral example is tightened to drop the superseded "drop-in" wording (no behavioral change — wording accuracy only). The Makefile itself changes no existing requirement.
...
- **Spec delta (wording only):** `terminal-spatial-blocks` MODIFIED requirement; its established-spec Purpose hand-fixed at archive. `.gitignore` gains `/build/` (the Makefile's DerivedData).
