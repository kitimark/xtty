...
- Which of `HostName` / `LocalHostName` / `ComputerName` backs bash `\h` inside the NAT'd Tart guest? Resolved empirically by D2 (set all three, verify the wrap) — not blocking the proposal, but the first apply task.
