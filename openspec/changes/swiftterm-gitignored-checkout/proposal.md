...
- **Bootstrap clones + enforces the pin.** `scripts/bootstrap-swiftterm.sh` becomes self-contained: clone `external/SwiftTerm` if absent, idempotently `fetch && checkout <UPSTREAM_REF>` (so the pin can't drift without re-running bootstrap), then drop in `XttyAccessors.swift`. One command does everything (no separate `submodule update --init`).
- **Keep everything else identical.** `XttyCore/Package.swift` still points at `.package(path: "../external/SwiftTerm")`; the add-only `cp` of `XttyAccessors.swift` is unchanged (a `git apply` of a `.diff` is only needed when *modifying* existing source — ours is a pure add). The build is unaffected: `bootstrap` then build, same as today.
