...
### D1 — Census mechanism: `#if DEBUG nonisolated(unsafe) static var liveCount`, plain `Int`, no atomics
Each lifecycle type gets a `#if DEBUG nonisolated(unsafe) static var liveCount = 0`, `+= 1` in `init`, `-= 1` in `deinit`. A plain `Int` (no atomics dependency) because every counter is touched only on the main thread — the objects are created and destroyed there. **Why `nonisolated(unsafe)` rather than the originally-planned plain `@MainActor static Int`:** Swift's `deinit` is **nonisolated**, so it cannot mutate a `@MainActor`-isolated static (compile error), and a bare nonisolated mutable static is flagged "not concurrency-safe." `nonisolated(unsafe)` with the documented single-owner/main-thread vouch — the exact pattern `GlobalHotKey` already uses for its Carbon refs — lets both `init` and the nonisolated `deinit` touch the counter cleanly. It is DEBUG-only, so a worst-case miscount never affects shipping behavior. *Alternatives considered:* plain `@MainActor static Int` (doesn't compile from `deinit`); `swift-atomics`/`OSAtomic` (needless dependency + overhead for serially-accessed counters).

### D2 — `TerminalSession` counted from `PaneController`, to keep `XttyCore` view/thread-assumption free
`TerminalSession` lives view-free in `XttyCore` and is intentionally non-isolated. It is created/destroyed **only** inside `PaneController` (on the main actor). Add a `#if DEBUG nonisolated(unsafe) static` count + `recordInit()/recordDeinit()` to `TerminalSession`, called by `PaneController` from its main-actor `init`/`deinit` — so `TerminalSession` itself makes **no** threading assumption in its own `init`/`deinit` (the count is driven by its main-actor owner). The `nonisolated(unsafe)` carries the same single-thread vouch as D1. *Alternative considered:* increment/decrement inside `TerminalSession.init/deinit` directly — rejected to avoid the view-free package's lifecycle asserting a thread.
