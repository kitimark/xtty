...
**Delivery check:** read `.claude/agents/xtty-openspec-critic.md`'s own **Definition version** line (line 9, under the `# xtty openspec critic` heading) to get the current authoritative stamp — do not hardcode or recall a specific version string here, since it is bumped on every edit to that file and a hardcoded copy in this launcher would itself go stale on the next bump (as it already has once). The report's first line must match that stamp exactly. A missing or mismatched stamp means the harness served a stale cached definition (agent-definition edits propagate to spawns with unpredictable lag — measured on the sibling agents: stale 63 s after an edit, fresh ~40 min later; a fresh session always serves current) — treat that as a blocker finding, not a valid run.
