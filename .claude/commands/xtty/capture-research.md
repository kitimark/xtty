...
It walks the capture-and-reconcile checklist: write the settled research/decision into the right `research/` subfolder (Provenance + Sources + ✅/❌/❓ conventions; a **dated addendum**, not a rewrite, for an evolving decision) — **to the capture-depth bar** when the finding has measured claims or retired theories (mechanism, reproducible probes, retired-theory fates, re-verify-by-effect, guideline if it generalizes) → index it in `research/README.md` → reconcile the trackers (`AGENTS.md` Current-status — for an **open (in-flight) change** there is no row to update, just the derive-on-demand pointer (`openspec list`/`openspec show <name>`); refresh the **snapshot** only if posture moved, bounded, never a narrative paragraph; the snapshot never caches the **test envelope**, whose single home is `packer/README.md` → Acceptance → Current envelope — plus the full narrative **appended to `HISTORY.md`** + `research/04-design/02-milestones.md`; a settled refutation is **one line** — conclusion + applicability condition + evidence pointer — and a change that retires/supersedes a refutation's mechanism **edits that entry in place**, never appends) → **verify against disk** (`ls openspec/changes/archive/` · `ls openspec/specs/` · `packer/README.md` → Acceptance) → commit only when asked.
