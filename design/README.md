...
| `xtty-mockups/` | The **project folder** — the design agent's working directory | Imported once by `make design-link` via the app's bundled CLI (GUI folder picker is the fallback). The app-side project is *named* **`xtty`** (display only — see the naming note below). Unlike the package basename, this folder's basename is **not** identity: the app stores only the absolute `baseDir` and resolves the project by realpath, never by name — but that stored path is immutable after import, so renaming this folder still means `make design-unlink` first, then `make design-link` (see Update lifecycle). |
...
`make design-link` now does the whole setup: it registers the package, then creates the **`xtty`** project (named at import via `--name`; working folder `design/xtty-mockups/`) through the app's **bundled first-party CLI** (`od project import-folder`, run via the app's own Electron helper — it mints the gated import token itself; the script never touches the daemon's socket or secrets) and points it at the `xtty` design system. It dedupes by `baseDir` first, so re-running is a no-op that reports the existing project — and if the existing project carries a different display name (a GUI import defaults to the folder name `xtty-mockups`), it renames it in place, keeping its id and run history. If any of that fails, the script prints the manual GUI steps, which remain the documented fallback:

1. **New project → "Open folder" →** `design/xtty-mockups/`
...
> The native folder panel opens at its **last-remembered location** — neither picker implementation pins a default directory — so navigate to `design/xtty-mockups/` explicitly and check the created project's `baseDir` afterwards (`make design-status` reports it). The historical mis-import that rooted a project one level too deep was this remembered-state capture, not a designed default; forensics doc, addendum (g).

> **The naming note.** The project's *name* is `xtty`; its *folder* is `design/xtty-mockups/` (the folder is the agent's working directory; the package folder is `xtty-design-system/` — its basename is identity, see the table above — and the `xtty-` prefix on both is an owner consistency call, 2026-07-29: the `design/` tree reads with one prefix; for the mockups folder the basename is cosmetic, not identity). The name/folder mismatch is deliberate and harmless: the name is pure display — it labels the app's project list and export filenames, never reaches the agent's prompt, and keys nothing (identity everywhere, in the app and in the script, is `realpath(baseDir)`). Two caveats it buys: (1) once the app has ensured the design-system *workspace* row (`ds-xtty-design-system`), the project list shows **two rows displaying "xtty"** — that row mirrors the design system's title; the one whose card opens `design/xtty-mockups/` is the project. (2) **Never rename the `ds-xtty-design-system` workspace row in the app** — a rename on a workspace row writes the new title through into the design system's `metadata.json`, which is our symlinked `design/xtty-design-system/metadata.json`, i.e. an app-initiated write into the git tree. Renaming the `xtty` project itself cannot do that (verified: the write-through is gated on `importedFrom:"design-system"`; ours is `"folder"`).
...
**Images:** a mockup is one self-contained file — an image a scenario genuinely cannot do without is inlined as a data URI. There is deliberately no `assets/` directory: nothing writes one (`od media generate` lands in the app's own data dir, never in this folder, and cannot address a subdirectory; uploads land flat), no mockup references one, and an empty subfolder was the one thing the remembered-state folder panel could mis-root a project into. Forensics doc, addendum (g).

**Revision copies** (`*-v2.html` and friends) are run-local scratch. They are deliberately **not** gitignored — hiding them would let the good revision sit in an invisible file while `git status` reads clean. Before committing: promote the winner over the canonical filename, delete the rest, then add or refresh its card in `xtty-mockups/index.html`. Rejected proposals get deleted; git is the archive.
...
Renaming `xtty-mockups/` itself is different: the project row stores an **absolute** `baseDir` that is **immutable after import** — the app's update route rejects any re-point (`baseDir is immutable after import; use a new import to change it`) and the CLI has no move verb — and teardown finds its target by that same stored path. So run `make design-unlink` **before** the `git mv` (a rename-first would orphan a row the script could no longer find), then `make design-link` after. Nothing else keys on the folder's basename (the name is set explicitly, `entryFile` is stored relative, exports name themselves after the title, runs reference the project id), and the current row's app-side history is all that recreation costs.
